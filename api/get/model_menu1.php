...
if($node == "db") {
    ...
    $dbUrl = isset($nodeDb["url"]) ? $nodeDb["url"] : null;
    $dbLink = isset($nodeDb["link"]) ? $nodeDb["link"] : null;
    $dbPa = isset($nodeDb["pa"]) ? $nodeDb["pa"] : null;
    ...
    $dbOpl = isset($nodeDb["opl"]) ? $nodeDb["opl"] : null;
    ...
    $moreNumberItem = isset($nodeMore["numberItem"]) && !empty($nodeMore["numberItem"]) ? $nodeMore["numberItem"] : 12;
    ...
    foreach ($multiLanguage as $key => $value) {
        ...
        array_push($groupContentCustom, [
            "iLabel"=>"{$language["contentCustom"]} ({$key})",
            "iName"=>"more.{$key}.contentCustom",
            "iValue"=>isset($nodeMore[$key]["contentCustom"]) ? $nodeMore[$key]["contentCustom"] : null,
            "iTypeInput"=>"input",
            "iTextarea"=>"true",
            "gClass"=>"col-xs-12 mb-10",
            "iClass"=>"form-control mce-editor"
        ]);
        array_push($groupNoteDetail, [
            "iLabel"=>"Note for Product ({$key})",
            "iName"=>"more.{$key}.noteDetail",
            "iValue"=>isset($nodeMore[$key]["noteDetail"]) ? $nodeMore[$key]["noteDetail"] : null,
            "iTypeInput"=>"input",
            "iTextarea"=>"true",
            "gClass"=>"col-xs-12 mb-10",
            "iClass"=>"form-control mce-editor"
        ...
        if($dbOpp == 3 || $dbOpp == 2) {
            $arrayPlus = array(
                ...
                array(
                    ...
                    "iPlaceholder"=>$language["itemNumber"],
                    "iClass"=>"form-control"
                ),
                array(
            ...
            $arrayPlus = array(
                array(
                    ...
                    "iDesign"=>'<div class="radio-inline"><label class="radio"><input type="radio" name="more.isform" data-key="more.isform" value="{0}" {2}><span class="fa radio-style"></span><span>{1} '.$language["contact"].'</span></label> </div>',
                    "iList"=>$language["dropdownLocalOption"]["yesNoQuestion"],
