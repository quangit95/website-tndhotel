<?php
$file = FOLDERPRODUCT . "product.xml";
$itemList = null;
if (is_file($file)) {
    $itemList = simplexml_load_file($file);
    $itemList = json_encode($itemList);
    $itemList = json_decode($itemList, true);
}
$nodeUpdate = isset($post["updateNode"])? $post["updateNode"]:null;

# generator javascript products file and save to asset folder
$dirAssets = FOLDERDATAOFWEBSITE."assets/";
$fileAccessJs = $dirAssets."products_{$dataOfWebsiteId}.js";


if($nodeUpdate == "db") {
    $infoUpdate = isset($post[$nodeUpdate]) ? $post[$nodeUpdate] : null;
    $iId = 0;
    if (!$itemList) {
        $iId = 1;
    } else {
        $table = $itemList["table"];
        // edit Item
        if (isset($infoUpdate["id"]) && intval($infoUpdate["id"]) > 0) {
            $iId = intval($infoUpdate["id"]);
        } else {
            // add Item
            $endElmTable = end($table);
            $iId = intval($endElmTable["id"]) + 1;
        }
    }
    $node = 'id_' . $iId;
    if(isset($infoUpdate["attr"]) && $infoUpdate["attr"]) {

        foreach ($infoUpdate["attr"] as $key => $value) {
            if(is_array($value)) {
                $infoUpdate["attr"][$key] = implode($value,",");
            }
        }

        $itemList["table"][$node]["attr"] = $infoUpdate["attr"];
    }
    elseif(isset($infoUpdate["pl"])) {
        $listPrice = isset($itemList["table"][$node]["pl"]) ? $itemList["table"][$node]["pl"] : array();

        if(isset($infoUpdate["pl"]["delId"])) {
            $delId = $infoUpdate["pl"]["delId"];
            if(isset($itemList["table"][$node]["pl"]["id_{$delId}"])) {
                unset($itemList["table"][$node]["pl"]["id_{$delId}"]);
            }
        } else {
            $priceListId = 1;
            if(count($listPrice) > 0 ) {
                if($infoUpdate["pl"]["id"]) {
                    $priceListId = $infoUpdate["pl"]["id"];
                } else {
                    $listPrice = end($listPrice);
                    $priceListId = intval($listPrice["id"]) + 1;
                }
            } else {
                $priceListId = 1;
            }
            $infoUpdate["pl"]["id"] = $priceListId;
            $itemList["table"][$node]["pl"]["id_{$priceListId}"] = $infoUpdate["pl"];
        }
    } else {
        if(isset($infoUpdate["st"]) && intval($infoUpdate["st"])){
            $infoUpdate["st"] = intval($infoUpdate["st"]);
        }
        else {
            $infoUpdate["st"] = 1;
        }

        if(isset($infoUpdate["cat"]) && is_array($infoUpdate["cat"])) {
            $infoUpdate["cat"] =  implode($infoUpdate["cat"], ',');
        }

        if(isset($infoUpdate["pcm"]) && is_array($infoUpdate["pcm"])) {
            $infoUpdate["pcm"] =  implode($infoUpdate["pcm"], ',');
        }


        if(isset($infoUpdate["pr"]["sale"]) && $infoUpdate["pr"]["sale"]) {
            $infoUpdate["pr"]["sale"] = str_replace(',','', $infoUpdate["pr"]["sale"]);
            $infoUpdate["pr"]["sale"] = floatval($infoUpdate["pr"]["sale"]);
        }
        if(isset($infoUpdate["pr"]["off"]) && $infoUpdate["pr"]["off"]) {
            $infoUpdate["pr"]["off"] = str_replace(',','', $infoUpdate["pr"]["off"]);
            $infoUpdate["pr"]["off"] = floatval($infoUpdate["pr"]["off"]);
            if(isset($infoUpdate["pr"]["sale"])) {
                $infoUpdate["pr"]["save"] = $infoUpdate["pr"]["sale"] - $infoUpdate["pr"]["off"];
                $infoUpdate["pr"]["per"] = round($infoUpdate["pr"]["save"]/$infoUpdate["pr"]["sale"],2)*100;
            }
        }

        // set id for post
        $infoUpdate["id"] = $iId;
        // update row before save
        foreach ($infoUpdate as $key => $value) {
            $itemList["table"][$node][$key] = $value;
        }
    }

    // save item to file
    if($itemList["table"][$node]["id"] && $itemList["table"][$node]["ti"]) {
        if (saveXMLFile($file, $itemList)) {
            $fileInfo = FOLDERPRODUCT . $iId . ".xml";
            if (is_file($fileInfo)) {
                $information = simplexml_load_file($fileInfo);
                $information = json_encode($information);
                $information = json_decode($information, true);
            }

            $information["db"] = $itemList["table"][$node];

            // save file detail
            if (saveXMLFile($fileInfo, $information)) {
                $dataResponse["table"][$node] = $post;
                $code = 200;
                $message = $language["updateSuccess"];
            } else {
                $code = 501;
                $errors = $language["unknownErrors"];
            }
        }
        $pf = fopen ($fileAccessJs, "w");
        fwrite ($pf, "window.localProductList=".json_encode(array_values($itemList["table"]), true) );
        fclose ($pf);

    } else {
        $code = 501;
        $errors = $language["unknownErrors"];
    }
} elseif($nodeUpdate == "del" && isset($post["id"])) {
    # delete node
    $iId = $post["id"];
    $node = 'id_' . $iId;

    try {
        # remove item from file database
        if(isset($itemList["table"][$node])) {
            $strImg = isset($itemList["table"][$node]["im"]) && count($itemList["table"][$node]["im"])>0? strval($itemList["table"][$node]["im"]):null;

            unset($itemList["table"][$node]);

            if (saveXMLFile($file, $itemList)) {
                $fileInfo = FOLDERPRODUCT . $iId . ".xml";
                $fileImage = FOLDERIMAGEPRODUCT . $strImg;
                # delete detail file
                if (is_file($fileInfo)) {
                    unlink($fileInfo);
                }
                # delete image file
                if (is_file($fileImage)) {
                    unlink($fileImage);
                }
                # delete folder slide
                $folderDel =FOLDERSLIDEPRODUCT.$iId;
                if(is_dir($folderDel)) {
                    deleteDirectory($folderDel);
                }

                $pf = fopen ($fileAccessJs, "w");
                fwrite ($pf, "window.localProductList=".json_encode(array_values($itemList["table"]), true) );
                fclose ($pf);

                $code = 200;
                $message = $language["updateSuccess"];
            }
        }
        else {
            $code = 404;
            $errors = "Not found ITEM";
        }
    } catch (Exception $ex) {
        $code = 501;
        $errors = $language["unknownErrors"];
    }

} elseif (isset($post["id"]) && $post["id"]) {
    $iId = $post["id"];
    $fileInfo = FOLDERPRODUCT . $iId . ".xml";
    $type = $nodeUpdate;

    if (is_file($fileInfo)) {
        $information = simplexml_load_file($fileInfo);
        $information = json_encode($information);
        $information = json_decode($information, true);
    }
    if(isset($post["delId"]) && $post["delId"] && $type=="detail") {

        // delete detailmore node
        $strNode = $post["delId"];
        if(isset($information[$type]["n_{$strNode}"])) {
            unset($information[$type]["n_{$strNode}"]);
        }
        // save file
        saveXMLFile($fileInfo, $information);

    } elseif (isset($post[$type]) && $post[$type] ) {

        if($type=="detail") {
            $strNode = 1;
            $listDetail = isset($information[$type]) ? $information[$type]:null;
            if (isset($post[$type]["id"]) && $post[$type]["id"]) {
                $strNode = $post[$type]["id"];
            } elseif ($listDetail) {
                $lastobj = end($listDetail);
                $strNode = intval($lastobj["id"]) + 1;
            }

            $post[$type]["id"] = $strNode;
            $information[$type]["n_{$strNode}"] = $post[$type];
        }
        else {
            /*foreach ($post[$type] as $key => $value) {
                $information[$type][$key] = $value;
            }*/
            $information[$type] = $post[$type];
        }

        // save file
        saveXMLFile($fileInfo, $information);
        $code = 200;
        $message = $language["updateSuccess"];
    } else {
        $code = 401;
        $errors = "invalidate form";
    }
}

?>
