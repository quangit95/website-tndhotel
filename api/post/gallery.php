...
if ($isUpdate) {
    ...
    if($type) {
        ...
        if($iId) {
            $fileDetail = FOLDERGALLERY . "{$iId}.xml";
        ...
        if(isset($infoUpdate["id"]) && $infoUpdate["id"] ) {
            ...
            if ($db->db_update($infoUpdate, TABLE_GALLERY, array("id" => $iId, "uid" => $uid))) {
            ...
            if ($db->db_insert($infoUpdate, TABLE_GALLERY)) {
                $str_query = "SELECT * FROM ".TABLE_GALLERY." WHERE cr={$currentTime} ORDER BY id DESC LIMIT 0,1";
                $row = $db->db_array($str_query);

                if ($row) {
                    $file = FOLDERGALLERY . "{$row["id"]}.xml";
                    $information = array(
                        "db" => $row,
                    );
                    saveXMLFile($file, $information);
                    ...
                    if(isset($infoUpdate["me"]) && isset($infoUpdate["pid"]) ){
                        #goto edit news
                        $dataResponse = array("urlRedirect" => $_SERVER['HTTP_REFERER']."&node=editgallery&id={$row["id"]}");
                    }
                }
                else {
                    $code = 401;
        ...
        $iId = isset($post["db"]["id"]) ? $post["db"]["id"]:null;
        if($iId) {
            if($db->db_delete(TABLE_GALLERY, array("id"=>$iId)) ){
                # delete image, xml, slide file
                $fileXml = FOLDERGALLERY . "{$iId}.xml";
                $dirSlide = FOLDERSLIDEGALLERY . "{$iId}";
                $fileImage = "";
                if (is_file($fileXml)) {
                    // delete image file
