...
$fileAccessJs = $dirAssets."blogs_{$dataOfWebsiteId}.js";
...
if($nodeUpdate == "db") {
    ...
    if (saveXMLFile($file, $itemList)) {
        ...
        fwrite ($pf, "window.localBlogList=".json_encode(array_values($itemList["table"]), true) );
        fclose ($pf);
    ...
    try {
        ...
        if(isset($itemList["table"][$node])) {
            ...
            if (saveXMLFile($file, $itemList)) {
                $fileInfo = FOLDERBLOG . $iId . ".xml";
                $fileImage = FOLDERIMAGEBLOG . $strImg;
                # delete detail file
                if (is_file($fileInfo)) {
                    unlink($fileInfo);
                }
