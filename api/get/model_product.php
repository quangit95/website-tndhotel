<?php
if($multiLanguage) {
    require_once dirname(__FILE__)."/model_product1.php";
} else {
    ...
    if($node == "db") {
        ...
        $priceSale = isset($dbPrice["sale"]) && floatval($dbPrice["sale"]) > 0 ? floatval($dbPrice["sale"]) : 0;
        $priceOff = isset($dbPrice["off"]) && floatval($dbPrice["off"])>0 ? floatval($dbPrice["off"]) : null;
        ...
        $moreDescription = isset($nodeMore["description"]) ? $nodeMore["description"] : null;
        $moreContent = isset($nodeMore["content"]) ? $nodeMore["content"] : null;
        ...
        $nodeData = isset($information[$node]) ? $information[$node] : [];
        $formUpdate =  array(
            array(
                "iLabel"=>"Option Size",
                "iName"=>"variantsize",
                "iTypeCheckbox"=>"checkbox",
                "iOptioned"=>isset($nodeData["id"]) ? json_decode(json_encode($nodeData["id"])):null,
                "iAttr"=>'data-validate data-pattern-message="invalid option" data-hidden-message="true" type="checkbox" data-option-onlys="2"',
                "iDesign"=>'<div class="checkbox-inline"><label class="checkbox"><input type="checkbox" name="{3}.{0}" data-key="{3}" value="{0}" {2}><span class="fa checkbox-style"></span><span> {1}</span></label> </div>',
                "iList"=>"languageText.websiteConfigToJson.variantSize",
                "iKey"=>"id",
                "iValue"=>"ti",
                "iKeyBox"=>"variantsize.id"
            )
        );
        $modeDesign = array(
            "eDesign" => array(
        ...
        $formUpdate =  array(
            array(
                ...
                "iTypeCheckbox"=>"checkbox",
                "iOptioned"=>isset($nodeData["id"]) ? json_decode(json_encode($nodeData["id"])):null,
                "iAttr"=>'data-hidden-message="true" type="checkbox" data-option-onlys="2"',
                "iDesign"=>'<div class="checkbox-inline"><label class="checkbox"><input type="checkbox" name="{3}.{0}" data-key="{3}" value="{0}" {2}><span class="fa checkbox-style"></span><span> {1}</span></label> </div>',
                "iList"=>"languageText.websiteConfigToJson.variantColor",
                "iKey"=>"id",
                "iValue"=>"html",
                "iKeyBox"=>"variantcolor.id"
            )
        );
        $modeDesign = array(
            "eDesign" => array(
