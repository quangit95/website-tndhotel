...
if($nodeUpdate == "db") {
    ...
    if(!isset($infoUpdate["im"])) {
        if(!isset($infoUpdate["ism"]) ) {
            $infoUpdate["ism"] = 0;
        }
    }

    if(isset($infoUpdate["so"]) && intval($infoUpdate["so"])){
        $infoUpdate["so"] = intval($infoUpdate["so"]);
    }
    else {
        $infoUpdate["so"] = 0;
    }
    ...
    try {
        ...
        if(isset($itemList["table"][$node])) {
            ...
            if (saveXMLFile($file, $itemList)) {
                $fileInfo = FOLDERMENU . $iId . ".xml";
                $fileImage = FOLDERIMAGEMENU . $strImg;
                # delete detail file
                if (is_file($fileInfo)) {
                    unlink($fileInfo);
                }
